...
class OneStepSentence(nn.Module):
    def __init__(self, config):
        ...
        self.qa_output = nn.Linear(2*config.hidden_size)
        self.pad_id = 1

    def forward(self, features, sentence_indicator):
        output = self.bert(**features)
        context_embedding = output[0]  # full context embedding
        sentences = []
        for i in range(1, sentence_indicator[0][-2]+1):
        ...
        chain_logits = self.chain_classifier(sentences).squeeze(dim=-1)
